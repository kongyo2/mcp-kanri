...
Antigravity, and the `config.toml` snippet used by Codex — all with one
click.
...
コマンドや、Claude Desktop / Cursor / VS Code / Google Antigravity 用の
`mcpServers` JSON、Codex の `config.toml` までワンクリックで生成・コピー
できます。
...
- **MCP サーバ登録の一元管理**: stdio / Streamable HTTP / SSE の 3 トランスポート
  ...
  できます。
- **9 種類の出力フォーマットを自動生成**: 1 つの登録から下記の貼り付け先を
  すべて出力します。
...
for Claude Desktop / Cursor / VS Code / Google Antigravity, and the
...
- **Nine output formats generated automatically**: a single registration is
